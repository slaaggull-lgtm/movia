...
const Favorites = () => {
  ...
  const fetchItems = async () => {
    const { data } = await api.get("/watch/favorite");
    setItems(data);
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold mb-8">⭐ Favorilerin</h1>
      {loading ? (
        <p className="text-center py-10">Yükleniyor...</p>
      ) : items.length === 0 ? (
        <p className="text-center py-10 text-gray-500">Henüz favori eklemedin.</p>
      ) : (
